...
```{r  API_key_load, cache = TRUE, results = 'hide', echo = FALSE}
census_api_key("25f4f15b896d92ddbb4b02fdc8c281b9304e001b", 
               overwrite = TRUE, install = TRUE)
```

```{r  API_key_load_fake, eval = FALSE, echo = FALSE}
...
MatchbyNearestDate<-function(df1, df2, ID, Date1, Date2){
  z <- lapply(intersect(df1[,ID],df2[,ID]),function(id) {
    ...
    df1[,"indices"] <- sapply(df1[,Date1],
                              function(d) which.min(abs(df2[,Date2] - d)))
